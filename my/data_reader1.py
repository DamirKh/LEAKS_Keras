...
class ScadaDataFile(object):
    ...
    def __init__(self, path_to_file=''):
        ...
        if os.path.exists(self._path_to_file):
            self.__f = open(self._path_to_file, newline='', encoding='windows-1251')
            self.__csv_data_iterator = csv.reader(self.__f, dialect='excel-tab')
            # First rollover. Find Tags and calculate time
            for row in self.__csv_data_iterator:
                # VerboseFunc(row)
                if len(row) < 3: continue  # empty string
                if (row[0], row[2]) == StampWorld:
                    # tagname found
                    self.tags.add(row[1])
                else:
                    dt = parse_date(row[0])
                    if dt < self.time_start:  # found ealer then start time time
                        self.time_start = dt
    ...
    def import_data_from_csv(self):
        ...
        for row in self.__csv_data_iterator:
            ...
            if (row[0], row[2]) == StampWorld:
                ...
                tag_position = tags_list.index(current_tag)
            else:
                # data found
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(data_path, '2Dx2.meta')):
        ...
        s = ScadaDataFile(os.path.join(data_path, '2Dx2.txt'))
